...
| `module_request` | 用户确认的目标 AI 检索文档、当前项目或当前文档树内必要的同级入口或上层入口 | 模块目标有歧义；存在多个强匹配候选；只有弱匹配候选；没有候选且目标路径未确认；用户要求更新或创建但目标文件、写入动作或入口写入范围未确认；模块代码、文档根或目标文件位置无法判断；需要跨项目写入 | 需要用户确认目标文件、下一步动作或写入范围但未确认；无法定位模块代码或文档根；缺少可验证事实来源；用户只是纯查找 |
