...
| 4 | 按 [输入路由与格式基线](#2-输入路由与格式基线) 中的优先级确定本次可应用的规则 | 用户约束、完整文档习惯、片段边界或新文档默认基线已明确 |
...
仅在用户明确要求启用 [标题层级](references/markdown-examples.md#3-标题层级) 中的「两级连续编号约定」，或完整文档已经稳定采用该约定时生效。其他情况下，只报告不依赖缺失上下文即可确认的结构问题。
...
| `first_draft` | 第一稿直接遵守已加载规则，并按 [输出契约](#4-输出契约) 直接呈现；只有「外层围栏」条件成立时才增加围栏 | 不得先生成违规内容，再等待用户要求格式化或修正 |
| `before_delivery` | 输出前执行当前 `content_scope` 已加载 reference 的适用检查清单 | 仅当已加载 [chinese-copywriting.md](references/chinese-copywriting.md) 时检查中文正文引号；出现 `“”` 且不属于非阻塞例外时，改为 `「」`，嵌套引号使用 `『』`，然后重新检查 |
...
命中格式边界、范围、引用读取或语义保护问题时，读取 [格式处理反例](references/format-antipatterns.md)，按其中的 `required_behavior` 执行；正常任务跳过该 reference。
...
- 已在适用范围内完成中文正文引号检查：仅当加载 `chinese-copywriting.md` 时默认使用 `「」`，嵌套使用 `『』`；用户指定风格和技术内容作为非阻塞例外保持不变
