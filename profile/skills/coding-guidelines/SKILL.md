...
description: 为编码任务提供行为约束与执行准则，在实现、调试、修复 bug、重构、补测试、代码审查等场景下减少主观猜测、避免过度设计、控制改动范围，并用清晰的验证目标驱动交付。当用户明确要求小步修改、最小改动、先澄清假设、避免顺手重构、按目标验证，或任务存在多种解释和实现路径时，使用该 Skill。
---
