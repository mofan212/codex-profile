...
| `issue_done` | Issue 实现完成，待验收 | 每条验收标准都有对应实现证据 |
...
存在外部 Review 结果时，只处理 Codex 收到 Review 结果后的门禁；Review 分类标准和处理规则详见 [references/readiness-checklists.md](references/readiness-checklists.md) 中的「Review 结果处理规则」。Issue DoD 只要求最近一轮 Review 没有阻塞级问题；非阻塞建议已处理或已有明确不处理理由即可。
