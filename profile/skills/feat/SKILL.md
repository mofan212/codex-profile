...
| 用户在已有 `feat` 需求目录、需求文档、Issue 或 `.issue/` 上下文中说「下一步」「继续 feat」「需求文档已完善」 | 作为续跑入口，先定位并读取现有产物，再判断当前阶段；没有可确认的需求文档路径时先询问路径 | 重新创建需求文档；猜测需求文档位置 |
...
阶段推进后的关键结论必须落到对应工作流产物中，使后续会话能从需求文档、Issue、`.issue/` 或 AI 检索文档恢复当前阶段；不要只依赖聊天上下文保存阶段状态。
