...
- 修改或新增 Skill 文档中的规则表格时，表头优先使用英文短字段，例如 `trigger`、`action`、`requirement`、`forbidden`、`purpose`；表格内容和正文使用中文。纯面向人类展示的清单可以使用中文表头。不要在同一张表中混用英文表头和中文表头
- 修改或新增 `profile/skills/*/references/` 下的表格时，路由表、契约表和机器字段表优先使用英文短表头；检查清单、示例分类、写作指南和命名说明优先使用中文表头；同一张表中不要混用英文表头和中文表头
